...
class InteractionModel:
    def __init__(self):
        ...
        self.agent = Agent(
            ...
            eps_alpha=self.creabot_config["eps_alpha"],
            beta=self.creabot_config["beta"],
        )
        self.time = 0
    ...
    def get_start_state(self):
        return CreabotState(
            ...
            ActionType.NEUTRAL,
            0,
            AgentDa(DialogActAgent.GREETING),
            Idea(IdeaQuality.NO),
        )
            ...
            for score in self.da.possible_idea_quality():
                for time in range(self.NTime + 1):
                    for a in range(N_ACTION):
                        states.append(
                            CreabotState(Mood(m), a, time, AgentDa(0), Idea(score))
                        )
        return states
    ...
    def interaction(self, verbose=True):
        ...
        while not is_terminal:
            ...
            is_terminal, state, action, reward = self.interaction_turn(
                state, verbose=verbose
            )
            belief_error.append(
                np.linalg.norm(self.agent.emotion_belief.belief_proba - self.user.Es)
            )
            if state.emotion is not None:
                emotion_error.append(
                    Emotion(
                        np.argmax(self.agent.emotion_belief.belief_proba)
                    ).distance_to(Emotion(np.argmax(self.user.Es)))
                )
                emotion_acc.append(
                    np.argmax(self.agent.emotion_belief.belief_proba)
                    == np.argmax(self.user.Es)
                )
            else:

                mood_acc.append(
                    np.argmax(self.agent.mood_belief.belief_proba)
                    == np.argmax(self.user.Es)
                )
    ...
    def get_next_state(self, state, action):
        ...
        if next_time != state.time_zone and next_time != self.NTime + 1:

            self.agent.current_user_transitions[
                :, :, next_time, :, :, :, next_time
            ] = self.agent.current_user_transitions[
                :, :, state.time_zone, :, :, :, state.time_zone
            ].copy()
            self.agent.current_user_reward[
                :, :, next_time, :, :, :, next_time
            ] = self.agent.current_user_reward[
                :, :, state.time_zone, :, :, :, state.time_zone
            ].copy()
        if state.emotion is not None:
            return is_terminal, CreabotState(
                emotion_state,
                action.bin_number,
                time // self.inte_t,
                AgentDa(self.agent.script.current_state),
                idea_quality,
            )
        else:
            return is_terminal, CreabotState(
    ...
    def get_reward(self, state, action, next_state):
        ...
        if state.emotion is not None:
            ...
            H = -np.sum(
                entropy(
                    self.agent.transitions[state.mood.bin_number][state.as_tuple()][
                        action.bin_number
                    ]
                )
            )
            reward = (
                next_state.idea_score.quality * 10
                + H
                - 1 * (action.bin_number == state.last_strat)
            )
        return reward
    ...
    def get_reward_no_entro(self, state, action, next_state):
        ...
        if state.emotion is not None:
            ...
            reward = next_state.idea_score.quality * 10 - 1 * (
                action.bin_number == state.last_strat
            )
        return reward
