...
from Script import DialogActUser, Idea, IdeaQuality
from utils import *
...
class User:
    def __init__(self):
        ...
        self.mood = np.array(
            [
                0.21 * self.Extraversion
                + 0.59 * self.Agreeableness
                + 0.19 * self.Neuroticism,
                0.15 * self.Openness
                + 0.30 * self.Agreeableness
                - 0.57 * self.Neuroticism,
                0.25 * self.Openness
                + 0.17 * self.Conscientiousness
    ...
    def print_emotion(self):
        print(
            "The user is in emotion : ",
            self.emotions[np.argmax(self.Es)].to_string(),
            "with intensity ",
            np.max(self.Es),
        )
